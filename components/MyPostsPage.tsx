...
import { View } from '../types'; // Changed: Import View as a value
...
export const MyPostsPage: React.FC<MyPostsPageProps> = ({
  ...
  const getStatusBadge = (item: UserPostItem) => {
    ...
    if (item.isHiredOrUnavailable) {
      return (
        <span className="inline-block bg-green-100 text-green-800 dark:bg-green-700/30 dark:text-green-300 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full">
          ✅ {item.type === 'job' ? 'ได้งานแล้ว' : 'ไม่ว่างแล้ว'}
        </span>
      );
    }
    if (item.isSuspicious) {
      return (
        <span className="inline-block bg-yellow-200 text-yellow-800 dark:bg-yellow-600/40 dark:text-yellow-300 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full">
          ⚠️ น่าสงสัย (รอตรวจสอบ)
        </span>
      );
    }
    if (isExpired) {
      return (
        <span className="inline-block bg-neutral-200 text-neutral-800 dark:bg-dark-border dark:text-dark-textMuted text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full">
          ⏳ หมดอายุ (ไม่แสดงในผลค้นหา)
        </span>
      );
    }
    return (
      <span className="inline-block bg-blue-100 text-blue-800 dark:bg-blue-700/30 dark:text-blue-300 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded-full">
        🟢 แสดงอยู่
      </span>
    );
  };
          ...
          {userItems.map(item => (
            <div key={`${item.type}-${item.id}`} className="bg-white dark:bg-dark-cardBg p-4 sm:p-6 rounded-lg shadow-lg border dark:border-dark-border">
              <div className="flex flex-col sm:flex-row justify-between sm:items-start mb-2">
                <div>
                    <h4 className={`font-semibold text-xl mb-1 ${item.type === 'job' ? 'text-primary dark:text-dark-primary-DEFAULT' : 'text-secondary-hover dark:text-dark-secondary-hover'}`}>
                    {item.title}
                    </h4>
                    <span className="text-xs text-neutral-medium dark:text-dark-textMuted">
                    {item.type === 'job' ? 'ประเภท: ประกาศงาน' : 'ประเภท: โปรไฟล์ผู้ช่วย'}
                    </span>
                </div>
                <span className="text-xs text-neutral-medium dark:text-dark-textMuted mt-1 sm:mt-0">
                  โพสต์เมื่อ: {formatDateDisplay(item.postedAt)}
                </span>
              </div>
              
              <div className="my-3">
                 สถานะ: {getStatusBadge(item)}
              </div>
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <Button
                  ...
                  variant="outline"
                  colorScheme="accent"
                  className="border-red-500 text-red-500 hover:bg-red-500 hover:text-white dark:border-red-400 dark:text-red-400 dark:hover:bg-red-400 dark:hover:text-neutral-dark w-full"
                  size="sm"
                >
                  🗑️ ลบโพสต์
                </Button>
