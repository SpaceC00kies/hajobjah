...
interface UserProfilePageProps {
  currentUser: User;
  onUpdateProfile: (updatedData: Pick<User, 'mobile' | 'lineId' | 'facebook' | 'gender' | 'birthdate' | 'educationLevel' | 'photo' | 'address' | 'favoriteMusic' | 'favoriteBook' | 'favoriteMovie' | 'hobbies' | 'favoriteFood' | 'dislikedThing' | 'introSentence'> & { photoFile?: File }) => Promise<boolean>;
  onCancel: () => void;
}
...
export const UserProfilePage: React.FC<UserProfilePageProps> = ({ currentUser, onUpdateProfile, onCancel }) => {
  ...
  const [photoPreview, setPhotoPreview] = useState<string | undefined>(currentUser.photo); // For display
  const [photoFile, setPhotoFile] = useState<File | undefined>(undefined); // For upload
  ...
  useEffect(() => {
    ...
    setPhotoFile(undefined); // Reset file on current user change
  ...
  const handlePhotoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) { // 2MB limit
        ...
        setPhotoPreview(currentUser.photo); // Revert preview
        return;
      }
      setPhotoFile(file); // Store the file object
      const reader = new FileReader();
      reader.onloadend = () => {
        setPhotoPreview(reader.result as string); // Show preview
      ...
      // User cancelled file selection, or wants to remove photo
      setPhotoFile(undefined);
      setPhotoPreview(undefined); // Clear preview if user deselects file
      setErrors(prev => ({ ...prev, photo: undefined }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await onUpdateProfile({ 
      ...
      photo: photoFile ? undefined : photoPreview, // Pass undefined if new file, else pass current preview (URL or old Base64)
      photoFile: photoFile, // Pass the file itself for Firebase upload
      address,
    ...
    if (success) {
      ...
      setPhotoFile(undefined); // Clear file after successful submission
    } else {
